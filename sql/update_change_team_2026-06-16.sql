-- =============================================================================
-- Atualização da master Change Team — 2026-06-16
-- =============================================================================
-- Objetivo: sincronizar lwsa.motor_change_team com a nova lista de 93 PRBs
-- recebida da coordenação em 2026-06-16.
--
-- Diff antecipado vs master 2026-06-10:
--   SAI    (1):  PRB0067965
--   ENTRAM (10): PRB0071972, PRB0071995, PRB0071997, PRB0072049, PRB0072162,
--                PRB0072164, PRB0072384, PRB0072738, PRB0072878, PRB0073007
--   Math:  84 - 1 + 10 = 93 ✓
--
...
-- -----------------------------------------------------------------------------
-- ETAPA 0 — Criar tabela temporária com a nova lista canônica de 93 PRBs
-- -----------------------------------------------------------------------------
...
INSERT INTO _change_team_nova_lista (numero) VALUES
    ...
    ('PRB0068236'), ('PRB0068319'), ('PRB0068344'), ('PRB0068547'),
    ('PRB0068880'), ('PRB0068888'), ('PRB0068958'), ('PRB0068961'),
    ('PRB0069348'), ('PRB0069465'), ('PRB0069543'), ('PRB0069607'),
    ('PRB0069725'), ('PRB0069746'), ('PRB0069777'), ('PRB0069940'),
    ('PRB0069964'), ('PRB0070155'), ('PRB0070280'), ('PRB0070421'),
    ('PRB0070457'), ('PRB0070718'), ('PRB0070735'), ('PRB0070861'),
    ('PRB0070862'), ('PRB0070869'), ('PRB0071148'), ('PRB0071149'),
    ('PRB0071228'), ('PRB0071253'), ('PRB0071604'), ('PRB0071643'),
    ('PRB0071665'), ('PRB0071758'), ('PRB0071783'), ('PRB0071791'),
    ('PRB0071961'), ('PRB0071972'), ('PRB0071979'), ('PRB0071995'),
    ('PRB0071997'), ('PRB0072049'), ('PRB0072062'), ('PRB0072088'),
    ('PRB0072104'), ('PRB0072152'), ('PRB0072162'), ('PRB0072164'),
    ('PRB0072175'), ('PRB0072228'), ('PRB0072260'), ('PRB0072274'),
    ('PRB0072311'), ('PRB0072340'), ('PRB0072363'), ('PRB0072365'),
    ('PRB0072384'), ('PRB0072400'), ('PRB0072524'), ('PRB0072583'),
    ('PRB0072648'), ('PRB0072691'), ('PRB0072705'), ('PRB0072729'),
    ('PRB0072738'), ('PRB0072878'), ('PRB0072925'), ('PRB0073006'),
    ('PRB0073007'), ('PRB0073011'), ('PRB0073051'), ('PRB0073198'),
    ('PRB0073350');
...
-- Esperado: 10 linhas → PRB0071972, PRB0071995, PRB0071997, PRB0072049,
--                       PRB0072162, PRB0072164, PRB0072384, PRB0072738,
--                       PRB0072878, PRB0073007
...
-- Esperado: 0 linhas se o usuário sempre faz incremental (o que sai não volta).
-- Se voltar algum, observação anterior é preservada (concatenada).
...
-- Esperado:
--   SAI    = 1  (PRB0067965)
--   ENTRA  = 10 (PRB0071972 + 9 outros)
--   REATIVA = 0 (provavelmente)
--
-- Se houver algo diferente, execute ROLLBACK; e me chame para investigar.
-- =============================================================================
...
SET ativo = false,
    ...
    observacao = COALESCE(observacao || ' | ', '')
              || 'Removido em 2026-06-16 — atualização da lista master (não consta na onda 2026-06-16).'
WHERE ativo = true
...
SET ativo = true,
    removido_em = NULL,
    observacao = COALESCE(observacao || ' | ', '')
              || 'Reativado em 2026-06-16 — atualização da lista master.'
WHERE ativo = false
...
SELECT n.numero,
       true,
       'Adicionado em 2026-06-16 — atualização da lista master (onda Change Team 2026-06-16).'
FROM _change_team_nova_lista n
WHERE n.numero NOT IN (SELECT numero FROM lwsa.motor_change_team);
...
-- Esperado: bate_total = true (93 ativos = 93 da nova lista)
